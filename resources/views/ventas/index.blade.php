                                    ...
                                    <select name="Documento_Empleado" class="form-select" required>
                                        ...
                                        @foreach($empleados as $emp)
                                            <option value="{{ $emp->Documento_Empleado }}"
                                                ...
                                                {{ $emp->Nombre_Usuario }} {{ $emp->Apellido_Usuario }} ({{ $emp->Documento_Empleado }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    async function iniciarExportacion(){
        ...
        try{
            ...
            const hoja1=todosLosDatos.map(v=>{
                const nombreCompleto=v.Nombre_Cliente||'';const partes=nombreCompleto.trim().split(' ');
                let nombre='',apellido='';
                if(partes.length===1){nombre=partes[0];}else if(partes.length>=2){nombre=partes[0];apellido=partes.slice(1).join(' ');}
                return{'Documento Cliente':v.Documento_Cliente,'Nombre Cliente':nombre,'Apellido Cliente':apellido,'Documento Empleado':v.Documento_Empleado};
            });
            const hoja2=[];
            todosLosDatos.forEach(v=>{(v.detalles||[]).forEach(d=>{hoja2.push({'Nombre Producto':d.Producto,'Cantidad':d.Cantidad,'Fecha Salida':d.Fecha_Salida});});});
            ...
            progresoDiv.className='alert alert-success';progresoDiv.innerHTML=`<i class="fa fa-check-circle"></i> <strong>¡Exportación completada!</strong><br><small>${todosLosDatos.length} ventas · ${hoja2.length} detalles</small>`;
