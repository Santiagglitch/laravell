            ...
            <div class="table-responsive mt-4">
                <table class="table table-bordered table-striped table-hover text-center align-middle">
                    <thead class="table-dark">
                        <tr>
                            <th>Documento</th>
                            <th class="col-ocultar-sm">Tipo Doc</th>
                            <th>Nombre</th>
                            <th class="col-ocultar-sm">Apellido</th>
                            <th class="col-ocultar-md">Edad</th>
                            <th class="col-ocultar-md">Correo</th>
                            <th class="col-ocultar-md">Teléfono</th>
                            <th class="col-ocultar-md">Género</th>
                            <th>Estado</th>
                            <th class="col-ocultar-sm">Rol</th>
                            <th class="col-ocultar-md">Foto</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                    @forelse($empleados as $emp)
                        <tr>
                            ...
                            <td class="col-ocultar-sm">{{ $emp->Tipo_Documento }}</td>
                            <td>{{ $emp->Nombre_Usuario }}</td>
                            <td class="col-ocultar-sm">{{ $emp->Apellido_Usuario }}</td>
                            <td class="col-ocultar-md">{{ $emp->Edad }}</td>
                            <td class="col-ocultar-md">{{ $emp->Correo_Electronico }}</td>
                            <td class="col-ocultar-md">{{ $emp->Telefono }}</td>
                            <td class="col-ocultar-md">{{ $emp->Genero }}</td>
                            <td>{{ $emp->estado->Nombre_Estado ?? 'Sin estado' }}</td>
                            <td class="col-ocultar-sm">{{ $emp->rol->Nombre ?? 'Sin rol' }}</td>
                            <td class="col-ocultar-md">
                                @if($emp->Fotos)
                                    @php
    ...
    async function importarDesdeExcel(event) {
        ...
        try {
            ...
            if (sinDocumento.length === datosValidados.length) {
                ...
                progresoDiv.innerHTML = `<i class="fa fa-exclamation-triangle"></i> <strong>Error: No se detectó la columna "Documento".</strong><br><small>Columnas encontradas: <code>${columnasReales.join(', ')}</code></small>`;
                event.target.value = '';
                return;
            }
            ...
            for (let i = 0; i < datosValidados.length; i += tamañoLote) {
                ...
                progresoDiv.innerHTML = `
                    <div class="d-flex align-items-center"><strong>Importando empleados...</strong><div class="ms-auto">${progreso}%</div></div>
                ...
                catch(e) { throw new Error('El servidor devolvió HTML. Verifica la ruta /migracion/empleados/importar.'); }
            ...
            let htmlFinal = `<i class="fa fa-check-circle"></i> <strong>¡Importación completada!</strong><br><small>Se importaron <strong>${importados}</strong> empleados correctamente.</small>`;
            if (todosLosErrores.length > 0) {
                htmlFinal += `<hr class="my-2"><small><strong>Advertencias (${todosLosErrores.length}):</strong><ul class="mb-0 mt-1 text-start">${todosLosErrores.map(e=>`<li>${e}</li>`).join('')}</ul></small>`;
            }
    ...
    async function iniciarExportacion() {
        ...
        try {
            ...
            while (!completado && intentos < 100) {
                ...
                progresoDiv.innerHTML = `
                    <div class="d-flex align-items-center"><strong>Exportando empleados...</strong><div class="ms-auto">${loteData.progreso}%</div></div>
            ...
            progresoDiv.innerHTML = `<i class="fa fa-check-circle"></i> <strong>¡Exportación completada!</strong><br><small>${todosLosDatos.length} empleados exportados</small>`;
