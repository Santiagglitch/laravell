...
<head>
    ...
    <title>Devoluciones - Empleado</title>
            ...
            <div class="table-responsive mt-4">
                <table class="table table-bordered table-striped table-hover text-center">
                    <thead class="table-dark">
                        <tr>
                            <th>ID Devolución</th>
                            <th class="col-ocultar-sm">Fecha Devolución</th>
                            <th>Motivo</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    ...
                    @forelse($devolucion as $dev)
                        <tr>
                            ...
                            <td class="col-ocultar-sm">{{ $dev->Fecha_Devolucion }}</td>
                            <td>{{ $dev->Motivo }}</td>
                            <td>
...
<script>
    ...
    async function importarDesdeExcel(event) {
        ...
        try {
            ...
            if (devs.length === 0) { progresoDiv.className='alert alert-warning'; progresoDiv.innerHTML='<i class="fa fa-exclamation-triangle"></i> El archivo está vacío'; return; }
            ...
            for (let i = 0; i < devs.length; i++) {
                ...
                for (const det of detsFila) {
                    ...
                    if (!ventaData.success) { errores.push(`Fila ${i+2}: Cliente ${docCliente} no encontrado.`); continue; }
                    if (cantDevuelta > ventaData.cantidad_maxima) {
                        errores.push(`Fila ${i+2}: Cliente ${docCliente} — cantidad ajustada al máximo (${ventaData.cantidad_maxima}).`);
                        detallesValidados.push({ ID_Venta: ventaData.ID_Venta, Cantidad_Devuelta: ventaData.cantidad_maxima });
                    } else {
                        detallesValidados.push({ ID_Venta: ventaData.ID_Venta, Cantidad_Devuelta: cantDevuelta });
                    }
                }
                datosValidados.push({ Fecha_Devolucion: fecha, Motivo: motivo, detalles: detallesValidados });
            }
            if (errores.length > 0) {
                erroresDiv.style.display = 'block';
                errores.forEach(e => { const li = document.createElement('li'); li.textContent = e; listaErrores.appendChild(li); });
            }
            ...
            for (let i = 0; i < datosValidados.length; i += tamañoLote) {
                ...
                progresoDiv.innerHTML = `<div class="d-flex align-items-center"><strong>Importando devoluciones...</strong><div class="ms-auto">${progreso}%</div></div><div class="progress mt-2"><div class="progress-bar progress-bar-striped progress-bar-animated bg-warning" style="width:${progreso}%"></div></div><small class="text-muted mt-2 d-block">Registros: ${i+lote.length} / ${datosValidados.length}</small>`;
                const response = await fetch('/migracion/importar', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ modulo: 'devoluciones', datos: lote }) });
                const texto = await response.text();
                let resultado;
                try { resultado = JSON.parse(texto); } catch(e) { throw new Error('El servidor devolvió HTML.'); }
            ...
            progresoDiv.innerHTML = `<i class="fa fa-check-circle"></i> <strong>¡Importación completada!</strong><br><small>Se importaron ${importados} devoluciones</small>`;
    ...
    async function iniciarExportacion() {
        ...
        btnExportar.disabled = true; btnExportar.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Exportando...';
        const progresoDiv = document.getElementById('progreso');
        try {
            progresoDiv.className = 'alert alert-info'; progresoDiv.innerHTML = '<i class="fa fa-spinner fa-spin"></i> Iniciando exportación...';
            const initResp = await fetch('/migracion/iniciar', { method: 'POST', headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ modulo: 'devoluciones' }) });
            ...
            while (!completado && intentos < 100) {
                const loteResp = await fetch('/migracion/lote', { method: 'POST', headers: { 'Content-Type':'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ modulo: 'devoluciones' }) });
                ...
                progresoDiv.innerHTML = `<div class="d-flex align-items-center"><strong>Exportando devoluciones...</strong><div class="ms-auto">${loteData.progreso}%</div></div><div class="progress mt-2"><div class="progress-bar progress-bar-striped progress-bar-animated bg-primary" style="width:${loteData.progreso}%"></div></div><small class="text-muted mt-2 d-block">Registros: ${loteData.registros_migrados} / ${loteData.total_registros}</small>`;
                completado = loteData.completado; intentos++; await new Promise(r => setTimeout(r, 300));
            }
            if (todosLosDatos.length === 0) { progresoDiv.className='alert alert-warning'; progresoDiv.innerHTML='<i class="fa fa-exclamation-triangle"></i> No hay datos'; btnExportar.disabled=false; btnExportar.innerHTML='<i class="fa fa-download"></i> Exportar a Excel'; return; }
            const hoja1 = todosLosDatos.map(dev => ({ 'Fecha Devolucion': dev.Fecha_Devolucion, 'Motivo': dev.Motivo }));
            const hoja2 = [];
            todosLosDatos.forEach(dev => { (dev.detalles ?? []).forEach(det => { hoja2.push({ 'Documento Cliente': det.Documento_Cliente ?? 'N/A', 'Cantidad Devuelta': det.Cantidad_Devuelta ?? 0, 'Cantidad Maxima': det.Cantidad_Maxima ?? 'N/A' }); }); });
            const wb = XLSX.utils.book_new();
            function estilos(ws, colorH, colorF) {
                const rng = XLSX.utils.decode_range(ws['!ref']); const cols = [];
                for (let C = rng.s.c; C <= rng.e.c; C++) { let w = 10; for (let R = rng.s.r; R <= rng.e.r; R++) { const c = ws[XLSX.utils.encode_cell({r:R,c:C})]; if (c?.v) w = Math.max(w, c.v.toString().length); } cols.push({wch: w+2}); }
                ws['!cols'] = cols;
                for (let C = rng.s.c; C <= rng.e.c; C++) { const a = XLSX.utils.encode_cell({r:0,c:C}); if (!ws[a]) continue; ws[a].s = { font:{name:'Calibri',sz:12,bold:true,color:{rgb:'FFFFFF'}}, fill:{fgColor:{rgb:colorH}}, alignment:{horizontal:'center'} }; }
                for (let R = rng.s.r+1; R <= rng.e.r; R++) { for (let C = rng.s.c; C <= rng.e.c; C++) { const a = XLSX.utils.encode_cell({r:R,c:C}); if (!ws[a]) continue; ws[a].s = { font:{name:'Calibri',sz:11}, fill:{fgColor:{rgb: R%2===0?'FFFFFF':colorF}}, alignment:{horizontal:'left'} }; } }
            }
            const ws1 = XLSX.utils.json_to_sheet(hoja1); estilos(ws1, '4472C4', 'F2F2F2'); XLSX.utils.book_append_sheet(wb, ws1, 'Devoluciones');
            if (hoja2.length > 0) { const ws2 = XLSX.utils.json_to_sheet(hoja2); estilos(ws2, 'ED7D31', 'FFF2CC'); XLSX.utils.book_append_sheet(wb, ws2, 'Detalles'); }
            const info = XLSX.utils.aoa_to_sheet([['REPORTE DE DEVOLUCIONES - TECNICELL RM'],[''],['Fecha de Generación:', new Date().toLocaleString('es-ES')],['Total Devoluciones:', todosLosDatos.length],['Total Detalles:', hoja2.length],['Generado por:', '{{ session("nombre") ?? "Empleado" }}']]);
            if (info['A1']) info['A1'].s = { font:{name:'Calibri',sz:16,bold:true,color:{rgb:'4472C4'}}, alignment:{horizontal:'center'} };
            info['!cols'] = [{wch:30},{wch:30}]; XLSX.utils.book_append_sheet(wb, info, 'Información');
            XLSX.writeFile(wb, `Devoluciones_${new Date().toISOString().split('T')[0]}.xlsx`, {bookType:'xlsx', cellStyles:true});
            progresoDiv.className = 'alert alert-success'; progresoDiv.innerHTML = `<i class="fa fa-check-circle"></i> <strong>¡Exportación completada!</strong><br><small>${todosLosDatos.length} devoluciones exportadas</small>`;
    ...
    function mostrarDetalles(data) {
        ...
        if (devolucion.detalles && devolucion.detalles.length > 0) {
            devolucion.detalles.forEach(detalle => {
                detallesHTML += `
                    <tr>
                        <td class="py-3"><i class="fa fa-box text-primary me-2"></i><strong>${detalle.Cantidad_Devuelta}</strong> unidades</td>
                        <td class="py-3"><i class="fa fa-receipt text-success me-2"></i>Venta #<strong>${detalle.ID_Venta}</strong></td>
                        <td class="py-3 text-center"><a href="${urlDetalleDevolucion}" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i> Editar</a></td>
                    </tr>`;
            });
        } else {
            detallesHTML = '<tr><td colspan="3" class="text-center text-muted py-5"><i class="fa fa-inbox fa-3x mb-3 d-block"></i><p>No hay detalles registrados</p></td></tr>';
        }
        document.getElementById('detalleModal').innerHTML = `
            <div class="modal-header bg-primary text-white py-3">
                <h5 class="modal-title"><i class="fa fa-info-circle me-2"></i> Detalle de Devolución - <span class="badge bg-light text-primary ms-2">ID: ${devolucion.ID_Devolucion}</span></h5>
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="d-flex align-items-start">
                                    ...
                                    <div><h6 class="text-muted mb-1 small">Fecha Devolución</h6><p class="mb-0 fw-bold fs-5">${devolucion.Fecha_Devolucion}</p></div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="d-flex align-items-start">
                                    <div class="bg-info bg-opacity-10 p-3 rounded me-3"><i class="fa fa-clipboard fa-lg text-info"></i></div>
                                    <div><h6 class="text-muted mb-1 small">Motivo</h6><p class="mb-0 fw-bold">${devolucion.Motivo}</p></div>
                                </div>
                            </div>
                        </div>
                    </div>
