...
<script>
    // ============================================
    // SIDEBAR RESPONSIVE (igual que el dashboard)
    // ============================================
        ...
        try {
            ...
            for (let i = 0; i < devs.length; i++) {
                ...
                for (const det of detsFila) {
                    ...
                    if (cantDevuelta > ventaData.cantidad_maxima) {
                        errores.push(`Fila ${i+2}: Cliente ${docCliente} — cantidad devuelta (${cantDevuelta}) supera el máximo (${ventaData.cantidad_maxima}). Se ajustó.`);
            ...
            for (let i = 0; i < datosValidados.length; i += tamañoLote) {
                ...
                progresoDiv.innerHTML = `
                    <div class="d-flex align-items-center"><strong>Importando...</strong><div class="ms-auto">${progreso}%</div></div>
    ...
    async function iniciarExportacion() {
        ...
        try {
            ...
            while (!completado && intentos < 100) {
                ...
                progresoDiv.innerHTML = `
                    <div class="d-flex align-items-center"><strong>Exportando...</strong><div class="ms-auto">${loteData.progreso}%</div></div>
            ...
            todosLosDatos.forEach(dev => {
                (dev.detalles ?? []).forEach(det => {
                    hoja2.push({ 'Documento Cliente': det.Documento_Cliente ?? 'N/A', 'Cantidad Devuelta': det.Cantidad_Devuelta ?? 0, 'Cantidad Maxima': det.Cantidad_Maxima ?? 'N/A' });
                });
            });
            ...
            function estilos(ws, colorH, colorF) {
                ...
                for (let C = rng.s.c; C <= rng.e.c; C++) {
                    ...
                    if (!ws[a]) continue;
                    ws[a].s = { font:{name:'Calibri',sz:12,bold:true,color:{rgb:'FFFFFF'}}, fill:{fgColor:{rgb:colorH}}, alignment:{horizontal:'center'} };
                }
                for (let R = rng.s.r+1; R <= rng.e.r; R++) {
                    for (let C = rng.s.c; C <= rng.e.c; C++) {
                        const a = XLSX.utils.encode_cell({r:R,c:C});
                        if (!ws[a]) continue;
                        ws[a].s = { font:{name:'Calibri',sz:11}, fill:{fgColor:{rgb: R%2===0?'FFFFFF':colorF}}, alignment:{horizontal:'left'} };
                    }
                }
            }
            ...
            progresoDiv.className = 'alert alert-success';
            progresoDiv.innerHTML = `<i class="fa fa-check-circle"></i> <strong>¡Exportación completada!</strong><br><small>${todosLosDatos.length} devoluciones exportadas</small>`;
