    ...
    <div class="contenido-principal flex-grow-1">
        ...
        <div class="container py-4">
            ...
            @if(session('mensaje'))
                ...
                <script>setTimeout(()=>{let a=document.getElementById('alertaMensaje');if(a){a.style.transition="opacity 0.5s";a.style.opacity=0;setTimeout(()=>a.remove(),500);}},3000);</script>
            @endif
            @if(session('error'))
                <div id="alertaError" class="alert alert-danger text-center">
                    <i class="fa fa-exclamation-circle me-2"></i>{{ session('error') }}
                </div>
                <script>setTimeout(()=>{let a=document.getElementById('alertaError');if(a){a.style.transition="opacity 0.5s";a.style.opacity=0;setTimeout(()=>a.remove(),500);}},4000);</script>
            @endif
            @if($errors->any())
                    ...
                    <div class="col-md-3">
                        <div class="card shadow-sm text-center p-4 h-100">
                            ...
                            <div class="text-start small">
                                <p class="mb-1"><i class="fa fa-id-card text-primary me-2"></i><strong>Documento:</strong> {{ $empleado->Documento_Empleado }}</p>
                                <p class="mb-1"><i class="fa fa-file-alt text-primary me-2"></i><strong>Tipo:</strong> {{ $empleado->Tipo_Documento }}</p>
                                <p class="mb-1"><i class="fa fa-birthday-cake text-primary me-2"></i><strong>Edad:</strong> {{ $empleado->Edad }}</p>
                                <p class="mb-1"><i class="fa fa-venus-mars text-primary me-2"></i><strong>Género:</strong> {{ $empleado->Genero }}</p>
                                <p class="mb-0">
                                    <i class="fa fa-circle text-{{ $empleado->ID_Estado == 1 ? 'success' : 'secondary' }} me-2"></i>
                                    <strong>Estado:</strong> {{ $empleado->ID_Estado == 1 ? 'Activo' : 'Inactivo' }}
                                </p>
                            </div>
                        ...
                        <div class="card shadow-sm p-4">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="Nombre_Usuario"
                                           ...
                                           value="{{ old('Nombre_Usuario', $empleado->Nombre_Usuario) }}" required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="Apellido_Usuario"
                                           ...
                                           value="{{ old('Apellido_Usuario', $empleado->Apellido_Usuario) }}" required>
                                    @error('Apellido_Usuario')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Correo Electrónico</label>
                                    <input type="email" name="Correo_Electronico"
                                           class="form-control campo-editable @error('Correo_Electronico') is-invalid @enderror"
                                           value="{{ old('Correo_Electronico', $empleado->Correo_Electronico) }}" required>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    @error('nueva_contrasena')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                                </div>
                                <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                @error('contrasena_actual')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                            </div>
...
<script>
    ...
    document.getElementById('inputFoto').addEventListener('change', function() {
        ...
        if (!file) return;
        if (file.size > 2 * 1024 * 1024) { alert('⚠️ La imagen no debe superar 2MB.'); this.value = ''; return; }
        const formatos = ['image/jpeg', 'image/png', 'image/webp'];
        if (!formatos.includes(file.type)) { alert('⚠️ Formatos permitidos: JPG, PNG, WEBP.'); this.value = ''; return; }
    ...
    document.getElementById('formPerfil').addEventListener('submit', function(e) {
        ...
        if (nueva && nueva !== confirma) { e.preventDefault(); alert('Las contraseñas nuevas no coinciden.'); return; }
        if (nueva && nueva.length < 8) { e.preventDefault(); alert('La nueva contraseña debe tener al menos 8 caracteres.'); }
    });
</script>
