        ...
        try {
            foreach ($datos as $index => $fila) {
                ...
                if (!$clienteExiste) {
                    ...
                    $estadoCliente = $fila['Estado_Cliente'] ?? 1;
                ...
                foreach ($detalles as $det) {
                    ...
                    $fechaSalida = date('Y-m-d'); // Valor por defecto
                    if (!empty($det['Fecha_Salida'])) {
                        // Si es un número (fecha serial de Excel)
                        if (is_numeric($det['Fecha_Salida'])) {
                            // Convertir número serial de Excel a fecha Unix
                            // Excel cuenta días desde 1900-01-01 (con bug del año 1900)
                            $excelEpoch = 25569; // Días entre 1900-01-01 y 1970-01-01
